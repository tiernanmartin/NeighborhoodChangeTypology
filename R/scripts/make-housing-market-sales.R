...
prep_parcel <- function(x, tax_year){
  x %>% dplyr::transmute(PIN = make_pin(MAJOR, MINOR),
                  TAX_YEAR = tax_year,
...
res_bldg_all <- purrr::pmap_df(list(res_bldg, tax_years), prep_res_bldg) %>%
  left_join(p_all, by = c("PIN", "TAX_YEAR"))
...
check_sales <- function(){
  ...
  sales %>%
    group_by(PROPERTY_CATEGORY, SALE_YEAR) %>%
    select(matches("SALE_PRICE")) %>%
    skimr::skim()
